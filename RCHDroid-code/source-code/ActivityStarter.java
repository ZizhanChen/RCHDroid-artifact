...
//------------------------------------------zizhan--------------------------------------------//
import static android.content.Intent.FLAG_ACTIVITY_ZIZHAN;
//------------------------------------------zizhan--------------------------------------------//
...
//------------------------------------------zizhan--------------------------------------------//
    private boolean mReusingZizhanActivity;
//------------------------------------------zizhan--------------------------------------------//
...
//------------------------------------------zizhan--------------------------------------------//        
        // 添加检查：查找是否有 ZizhanState 为 true 的 ActivityRecord
        ActivityRecord existingZizhanActivity = null;
        if (mStartActivity != null) {
            existingZizhanActivity = mRootWindowContainer.findZizhanActivity(
                mIntent, mStartActivity.info, false);
        }
        final Task reusedTask;
        if (existingZizhanActivity != null) {
            // 如果找到 ZizhanState 为 true 的 ActivityRecord，复用其所在的 Task
            reusedTask = existingZizhanActivity.getTask();
            // 设置标志，表示我们正在复用一个现有的 ActivityRecord
            mReusingZizhanActivity = true;
            //Slog.d(TAG, "[zizhan] Reusing existing Zizhan activity: " + existingZizhanActivity);
        } else {
            // 否则，使用正常的 Task 复用逻辑
            reusedTask = getReusableTask();
            mReusingZizhanActivity = false;
        }        
//------------------------------------------zizhan--------------------------------------------//
...
//------------------------------------------zizhan--------------------------------------------//
        // 如果复用了 Zizhan Activity，我们需要确保它被带到前台
        if (existingZizhanActivity != null && !newTask) {
            // 检查复用的 Activity 是否是启动器
            boolean isLauncher = existingZizhanActivity.packageName.equals("com.android.launcher3") ||
                         existingZizhanActivity.packageName.equals("com.google.android.apps.nexuslauncher");
            //Slog.d(TAG, "[zizhan] Reusing existing Zizhan activity: " + existingZizhanActivity);
                // 检查 Activity 的详细状态
            //Slog.d(TAG, "[zizhan] Activity state: " + existingZizhanActivity.getState());
            //Slog.d(TAG, "[zizhan] Activity visible: " + existingZizhanActivity.isVisible());
            //Slog.d(TAG, "[zizhan] Activity finishing: " + existingZizhanActivity.finishing);
            //Slog.d(TAG, "[zizhan] Activity focusable: " + existingZizhanActivity.isFocusable());
    
            // 检查任务栈状态
            //Slog.d(TAG, "[zizhan] Task: " + targetTask);
            //Slog.d(TAG, "[zizhan] Task visible: " + targetTask.isVisible());
            //Slog.d(TAG, "[zizhan] Task focused: " + targetTask.isFocused());
            
            if (isLauncher) {
                //Slog.w(TAG, "[zizhan] Avoiding reuse of launcher as Zizhan activity");
                // 不复用启动器，继续正常启动流程
                existingZizhanActivity = null;
                return START_SUCCESS;
            } else {
                // 确保 Zizhan Activity 所在的 Task 被带到前台
                targetTask.getRootTask().moveToFront("reuseZizhanActivity", targetTask);
        
                // 直接返回成功，因为我们复用了现有的 Activity，不需要创建新的
                // 但需要确保 Activity 可见并可能执行转场动画
                mTargetRootTask = targetTask.getRootTask();
                mTargetRootTask.ensureActivitiesVisible(existingZizhanActivity, 
                    0 /* configChanges */, !PRESERVE_WINDOWS);
                mTargetRootTask.mDisplayContent.executeAppTransition();
        
                // 记录日志等
                //mStartActivity.logStartActivity(EventLogTags.WM_REUSE_ACTIVITY, targetTask);
     
                // 确保 Zizhan Activity 被 resume
                //final Task focusedRootTask = mPreferredTaskDisplayArea.getFocusedRootTask();
                //ActivityRecord currentFocused = mRootWindowContainer.getTopResumedActivity();
                //if (currentFocused != existingZizhanActivity) {

                existingZizhanActivity.setVisible(true);

                targetTask.moveToFront("reuseZizhanActivity");
                boolean isTop = existingZizhanActivity.moveFocusableActivityToTop("reuseZizhanActivity");
                //Slog.d(TAG, "[zizhan] existingZizhanActivity.moveFocusableActivityToTop return " + isTop);
                
                boolean moved = mTargetRootTask.moveActivityToFront(existingZizhanActivity);
                //Slog.d(TAG, "[zizhan] mTargetRootTask.moveActivityToFront return " + moved);

                mRootWindowContainer.resumeFocusedTasksTopActivities(
                    mTargetRootTask, existingZizhanActivity, mOptions, mTransientLaunch);
                //}
                //Slog.d(TAG, "[zizhan] after all, Activity state: " + existingZizhanActivity.getState());
                //Slog.d(TAG, "[zizhan] Activity visible: " + existingZizhanActivity.isVisible());
                //Slog.d(TAG, "[zizhan] Activity finishing: " + existingZizhanActivity.finishing);
                //Slog.d(TAG, "[zizhan] Activity focusable: " + existingZizhanActivity.isFocusable());
                ActivityRecord focusedActivity = mRootWindowContainer.getTopResumedActivity();
                //Slog.d(TAG, "[zizhan] Currently focused activity: " + focusedActivity);
                //Slog.d(TAG, "[zizhan] return success with reusing existing zizhan activity: " + existingZizhanActivity);
                return START_SUCCESS;
            }
        }
//------------------------------------------zizhan--------------------------------------------//
...
//------------------------------------------zizhan--------------------------------------------//
        if (mStartActivity != null && sourceRecord != null && (mLaunchFlags & FLAG_ACTIVITY_ZIZHAN)!= 0) {
            //boolean isLauncher = mStartActivity.packageName.equals("com.android.launcher3") ||
            //        mStartActivity.packageName.equals("com.google.android.apps.nexuslauncher");
            //if (isLauncher){
            //    mStartActivity.setZizhanState(false);
            //} else {
                mStartActivity.setZizhanState(false);
                sourceRecord.setZizhanState(true);
                //Slog.d(TAG, "[zizhan] Set ZizhanState to false for activity: " + mStartActivity);
                //Slog.d(TAG, "[zizhan] Set ZizhanState to true for activity: " + sourceRecord);
            //}
        }
//------------------------------------------zizhan--------------------------------------------//
