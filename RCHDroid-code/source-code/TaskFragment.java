...
class TaskFragment extends WindowContainer<WindowContainer> {
    ...
    final boolean resumeTopActivity(ActivityRecord prev, ActivityOptions options,
        ...
        if (next == null || !next.canResumeByCompat()) {
            //Slog.d(TAG, "[zizhan] next == null or !next.canResumeByCompat, return fasle");
            return false;
        }
        ...
        if (!skipPause && !mRootWindowContainer.allPausedActivitiesComplete()) {
            ...
            //Slog.d(TAG, "[zizhan] resumeTopActivity: Skip resume: some activity pausing, return false");
            return false;
        }
        ...
        if (mResumedActivity == next && next.isState(RESUMED)
            ...
            ProtoLog.d(WM_DEBUG_STATES, "resumeTopActivity: Top activity "
                    + "resumed %s", next);
            //Slog.d(TAG, "[zizhan] resumeTopActivity: Top activity resumed [" + next + "] return false");        
            return false;
        }
        ...
        if (mLastPausedActivity == next && shouldSleepOrShutDownActivities()) {
            ...
            ProtoLog.d(WM_DEBUG_STATES, "resumeTopActivity: Going to sleep and"
                    + " all paused");
            //Slog.d(TAG, "[zizhan] resumeTopActivity: Going to sleep and all paused, return false");
            return false;
        }
        ...
        if (!mAtmService.mAmInternal.hasStartedUserState(next.mUserId)) {
            ...
            //Slog.d(TAG, "[zizhan] Skipping resume of top activity " + next
            //        + ": user " + next.mUserId + " is stopped, return false");
            return false;
        }
        ...
        if (pausing && !next.getZizhanState()) {
            ...
            //Slog.d(TAG, "[zizhan] resumeTopActivity: Skip resume: need to start pausing return true");
            return true;
            ...
            //Slog.d(TAG, "[zizhan] resumeTopActivity: Top activity resumed (dontWaitForPause) " + next);
            return true;
        }
        ...
        if (next.attachedToProcess()) {
            ...
            if (notUpdated) {
                ...
                //Slog.d(TAG, "[zizhan] Activity config changed during resume: "
            ...
            try {
                ...
                //Slog.d(TAG, "[zizhan] Resume failed; resetting state to: " + lastState 
                //        + ":" + next);
                return true;
            }
            ...
            try {
                ...
                //Slog.d(TAG, "[zizhan] Exception thrown during resume of " + next, e);
                return true;
            }
        } else {
            // Whoops, need to restart this activity!
            ...
            //Slog.w(TAG, "[zizhan] resumeTopActivity: Restarting" + next);
            mTaskSupervisor.startSpecificActivity(next, true, true);
        }
        //Slog.w(TAG, "[zizhan] finally return true");
        return true;
    }
